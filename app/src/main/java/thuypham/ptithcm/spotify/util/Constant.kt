package thuypham.ptithcm.spotify.util



const val BASE_URL = "http://string-api.vinova.sg/api/"

const val CURRENT_PER_PAGE = 20
const val ITEM_COUNT_BANNER = 5
const val ITEM_VIEW = 10
const val ITEM_COUNT_HISTORY = 3
const val VERSION_STRING = "1.0.1"

const val PREFS_NAME = "PREFERENCES"
const val FIRST_TIME = "FIRST_TIME"
const val EMAIL = "EMAIL"
const val USER_NAME = "USER_NAME"
const val PASSWORD = "PASSWORD"
const val ACCESS_TOKEN = "ACCESS_TOKEN"
const val USER_ID = "USER_ID"
const val DATABASE_NAME = "SPOTIFY"

// Error auth
const val ERR_EMAIL_VERIFY = "Email has not been verify!"
const val ERR_EMAIL_NOT_Exist = "Email does't exist!"
const val ERR_INCORRECT_PW = "Incorrect password!"
const val ERR_WEAK_PASSWORD =
    "The password is too weak, the password consists of at least 6 characters!"
const val ERR_EMAIL_INVALID = "Email invalidate!"
const val ERR_EMAIL_EXIST = "Email already exists!"

const val USER = "Users"
const val FOLLOW_ARTISTS = "FollowArtists"
const val ADVERTISE = "Advertises"
const val SONG = "Songs"
const val SONG_TYPE = "SongTypes"
const val COUNTRY = "Countries"
const val ALBUM = "Albums"
const val FAVORITE_ALBUM= "FavoriteAlbums"
const val PLAYLIST = "PlayLists"
const val COMMENT = "Comments"
const val REPLY_COMMENT = "replyComment"
const val ARTIST = "Artists"
const val FAVORITE_SONG = "FavoriteSongs"
const val HISTORY = "Histories"
const val FILTER_TYPE = "FilterType"
